...
export class MovieFormApp extends LitElement {
  ...
  login() {
    this.albums.loginId = this.$get("#loginId").value;

    this.albums.genre = this.$get("#genre").value;

    const error = this.$get("#error");

    if (this.albums.loginId && this.albums.genre) {
      this.postData("http://localhost:8080/user", this.albums);
    } else {
      ...
      <div class="container">
        <div id="login">
          ...
          <div class="login-block">
            ...
            <button @click=${this.login}>Update User</button>
          </div>
        </div>
      </div>
    `;
...
customElements.define("profile-form", MovieFormApp);
